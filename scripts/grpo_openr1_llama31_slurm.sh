...
# See docs/GRPO_OPEN_R1_RUNBOOK.md for --snip-objective lm vs dpo_preference and full paths.
...
#
# On many sites the gpu partition requires an explicit --gres; omitting it can yield
...
if not torch.cuda.is_available():
    print(
        ...
        "Under a GPU allocation, `python -c \"import torch; print(torch.cuda.is_available())\"` must print True.",
        file=sys.stderr,
    )
    sys.exit(1)
