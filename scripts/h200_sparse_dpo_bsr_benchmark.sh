#!/bin/bash
# Single H200 job: **dense baseline + full-grid** random-mask sparse BSR-DPO phases
# (see src/full_training/h200_sparse_dpo_bsr_benchmark.py). This wrapper does **not** pass
# ``--mlp_only`` (full-model mask scope; matches pipeline-style sparse DPO). Submit from repo root:
#   sbatch scripts/h200_sparse_dpo_bsr_benchmark.sh
#
# --- Why end-to-end [throughput] often differs only ~few % across phases ---
# 1) Masks apply to all scored Linears unless you run the Python driver manually with ``--mlp_only``.
...
#
# Site defaults; override env vars as needed.
...
# Canonical env on cluster scratch (sbatch --export=ALL can inherit a broken TRAIN_ENV, e.g.
...
# Steps **per phase** (default 25). Uses H200_BSR_STEPS_PER_PHASE only — we do NOT read NUM_STEPS_DPO here.
