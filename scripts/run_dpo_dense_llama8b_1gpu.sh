#!/bin/bash
# Single-GPU Dense DPO for Llama-3.1-8B (gpu partition, 8h limit).
# Use for canary runs (N_STEPS=100 via env) OR full 500-step training.
#
# Usage:
#   N_STEPS=100 RUN_TAG=canary sbatch scripts/run_dpo_dense_llama8b_1gpu.sh
#   RUN_TAG=dense_matched_1gpu sbatch scripts/run_dpo_dense_llama8b_1gpu.sh
#
#SBATCH --job-name=llama8b_dpo_1gpu
#SBATCH --output=logs/llama8b_dpo_1gpu_%j.out
#SBATCH --error=logs/llama8b_dpo_1gpu_%j.err
...
# Single GPU: no accelerate launch, run python directly. Keep effective batch
# size via gradient accumulation (grad_accum=8 matches the 4-GPU setup).
GRAD_ACCUM=8
LAUNCH="python"
...
[ $STATUS -eq 0 ] && echo "SUCCESS: $RUN_NAME" || echo "FAILED: $RUN_NAME (exit $STATUS)"
echo "=================================================="
exit $STATUS
