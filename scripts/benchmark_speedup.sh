#!/bin/bash
# ============================================================================
# Sparse Training Speedup Benchmark — Single H200 GPU
#
# Runs 4 training methods with identical hyperparams and collects timing JSON:
#   1. Dense + AdamW
#   2. Dense + SGD
#   3. LoRA + AdamW
#   4. Sparse BSR + SparseAdamW
#
# Submit: sbatch scripts/benchmark_speedup.sh
# Local:  bash scripts/benchmark_speedup.sh
# ============================================================================
#SBATCH --job-name=speedup_bench
#SBATCH --output=logs/speedup_bench_%j.out
#SBATCH --error=logs/speedup_bench_%j.err
#SBATCH --partition=gpu
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:h200:1
#SBATCH --mem=128G
#SBATCH --time=04:00:00

set -euo pipefail

# ── Repo root ───────────────────────────────────────────────────────────────
if [ -n "${SLURM_SUBMIT_DIR:-}" ] && [ -d "${SLURM_SUBMIT_DIR}" ]; then
    REPO_ROOT="$(cd "${SLURM_SUBMIT_DIR}" && pwd)"
else
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
fi
cd "$REPO_ROOT"
mkdir -p logs

echo "====== Speedup Benchmark ======"
echo "Date:     $(date)"
echo "Node:     $(hostname)"
echo "Job ID:   ${SLURM_JOB_ID:-local}"
echo "GPU:      ${CUDA_VISIBLE_DEVICES:-auto}"
echo "Repo:     $(pwd)"

# ── Environment ─────────────────────────────────────────────────────────────
CONDA_ENV_PRIMARY="/scratch/xie.yiyi/conda_envs/rl_casino"
CONDA_ENV_FALLBACK="/home/xie.yiyi/.conda/envs/rl_casino"

if [ -d "$CONDA_ENV_PRIMARY" ]; then
    ENV_PATH="$CONDA_ENV_PRIMARY"
elif [ -d "$CONDA_ENV_FALLBACK" ]; then
    ENV_PATH="$CONDA_ENV_FALLBACK"
else
    echo "ERROR: conda env not found"; exit 1
fi

PYTHON="$ENV_PATH/bin/python"
export PATH="$ENV_PATH/bin:$PATH"
export PYTHONPATH=.
echo "Env:      $ENV_PATH"

# ── Shared Config ───────────────────────────────────────────────────────────
MODEL="google/gemma-3-270m-it"
DATASET="math-220k"
N_STEPS=50
SUBSET=512
BATCH_SIZE=1
GRAD_ACCUM=8
NUM_GENERATIONS=8
GEN_BATCH_SIZE=8
LR=5e-6

MASK_PATH="masks/top_10.0pct_momentum_w25_step25.pt"

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTPUT_DIR="/scratch/biggs.s/rl_casino_outputs/benchmark_${TIMESTAMP}"
CACHE_DIR="/scratch/biggs.s/hf_cache/datasets"
mkdir -p "$OUTPUT_DIR"

PASS=0
FAIL=0
TOTAL=0

run_and_track() {
    local NAME="$1"; shift
    echo ""
    echo "============================================================"
    echo "[$((TOTAL+1))] $NAME"
    echo "============================================================"
    TOTAL=$((TOTAL + 1))
    if "$@"; then
        echo "  -> $NAME: PASSED"
        PASS=$((PASS + 1))
    else
        echo "  -> $NAME: FAILED"
        FAIL=$((FAIL + 1))
    fi
}

# ── 1. Dense + AdamW ────────────────────────────────────────────────────────
run_and_track "Dense + AdamW" \
    "$PYTHON" src/full_training/GRPO_timing_baseline.py \
        --model_name "$MODEL" \
        --n_steps "$N_STEPS" \
        --subset_size "$SUBSET" \
        --batch_size "$BATCH_SIZE" \
        --grad_accum "$GRAD_ACCUM" \
        --learning_rate "$LR" \
        --optimizer adamw \
        --use_wandb \
        --output_base_dir "$OUTPUT_DIR"

# ── 2. Dense + SGD ──────────────────────────────────────────────────────────
run_and_track "Dense + SGD" \
    "$PYTHON" src/full_training/GRPO_timing_baseline.py \
        --model_name "$MODEL" \
        --n_steps "$N_STEPS" \
        --subset_size "$SUBSET" \
        --batch_size "$BATCH_SIZE" \
        --grad_accum "$GRAD_ACCUM" \
        --learning_rate "$LR" \
        --optimizer sgd \
        --use_wandb \
        --output_base_dir "$OUTPUT_DIR"

# ── 3. LoRA + AdamW ─────────────────────────────────────────────────────────
run_and_track "LoRA + AdamW (r=16)" \
    "$PYTHON" src/full_training/lora_grpo_timing.py \
        --model_name "$MODEL" \
        --dataset "$DATASET" \
        --n_steps "$N_STEPS" \
        --subset_size "$SUBSET" \
        --batch_size "$BATCH_SIZE" \
        --grad_accum "$GRAD_ACCUM" \
        --num_generations "$NUM_GENERATIONS" \
        --generation_batch_size "$GEN_BATCH_SIZE" \
        --lr "$LR" \
        --optimizer adamw \
        --lora_rank 16 \
        --use_wandb \
        --output_base_dir "$OUTPUT_DIR" \
        --dataset_cache_dir "$CACHE_DIR"

# ── 4. Sparse BSR + SparseAdamW ─────────────────────────────────────────────
run_and_track "Sparse BSR + SparseAdamW" \
    "$PYTHON" src/full_training/sparse_grpo_bsr.py \
        --model_name "$MODEL" \
        --dataset "$DATASET" \
        --n_steps "$N_STEPS" \
        --subset_size "$SUBSET" \
        --batch_size "$BATCH_SIZE" \
        --grad_accum "$GRAD_ACCUM" \
        --num_generations "$NUM_GENERATIONS" \
        --generation_batch_size "$GEN_BATCH_SIZE" \
        --lr "$LR" \
        --optimizer sparse_adamw \
        --mask "$MASK_PATH" \
        --block_size_bsr 16 \
        --block_size_adam 128 \
        --use_wandb \
        --output_base_dir "$OUTPUT_DIR" \
        --dataset_cache_dir "$CACHE_DIR" \
        --save_model false

# ── Collect Results ──────────────────────────────────────────────────────────
echo ""
echo "============================================================"
echo "Collecting timing results..."
echo "============================================================"

"$PYTHON" -c "
import json, glob, os

out_dir = '$OUTPUT_DIR'
results = []
for f in sorted(glob.glob(os.path.join(out_dir, '*/timing_results.json'))):
    with open(f) as fh:
        d = json.load(fh)
        results.append(d)
        method = d.get('method', '?')
        opt = d.get('optimizer', '?')
        wall = d.get('wall_time', 0)
        per_step = d.get('time_per_step_wall', 0)
        gpu_time = d.get('gpu_time', 0)
        gpu_per = d.get('time_per_step_gpu', 0)
        extra = ''
        if 'block_size_bsr' in d:
            extra = f'  bsr={d[\"block_size_bsr\"]}'
        if 'lora_rank' in d:
            extra = f'  r={d[\"lora_rank\"]}'
        print(f'  {method:12s} {opt:12s}  wall={wall:7.1f}s  step={per_step:5.2f}s  gpu={gpu_time:7.1f}s  gpu/step={gpu_per:5.2f}s{extra}')

summary_path = os.path.join(out_dir, 'benchmark_summary.json')
with open(summary_path, 'w') as fh:
    json.dump(results, fh, indent=2)
print(f'\nSummary saved to {summary_path}')
"

echo ""
echo "============================================================"
echo "BENCHMARK COMPLETE"
echo "============================================================"
echo "Passed: $PASS / $TOTAL"
echo "Failed: $FAIL / $TOTAL"
echo "Output: $OUTPUT_DIR"
echo "Finished: $(date)"

[ "$FAIL" -gt 0 ] && exit 1 || exit 0
