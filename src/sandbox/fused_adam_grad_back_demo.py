...
# ============================================================================
# SPARSE GRADIENT COMPUTATION KERNEL
# ============================================================================
...
def sparse_grad_kernel(
    ...
    grad_ptr,
    # Inputs
    error_ptr,      # (M x N_samples) prediction error
    X_ptr,          # (N_features x N_samples) input
    mask_ptr,       # (M x N_features) sparsity mask
    # Dimensions
    M, N_features, N_samples,
    stride_gm, stride_gn,
    stride_em, stride_es,
    stride_xn, stride_xs,
    stride_mm, stride_mn,
    ...
    """
    Compute sparse gradient: grad = (error @ X.T) * mask / n_samples
    
    Only computes gradients for non-zero blocks in the mask.
    This is the KEY to getting sparse backward speedup.
    """
    # Get block position
    ...
    # Check if this block is active (sample center element)
    m_center = m_start + BLOCK_M // 2
    n_center = n_start + BLOCK_N // 2
    if m_center < M and n_center < N_features:
        block_mask = tl.load(mask_ptr + m_center * stride_mm + n_center * stride_mn)
        if block_mask == 0.0:
            # This block is masked - write zeros and return
            m_mask = (m_offsets < M)[:, None]
            n_mask = (n_offsets < N_features)[None, :]
            valid = m_mask & n_mask
            grad_offsets = m_offsets[:, None] * stride_gm + n_offsets[None, :] * stride_gn
            tl.store(grad_ptr + grad_offsets, 0.0, mask=valid)
            return
    
    # This block is active - compute gradient
    # grad[m, n] = sum_s (error[m, s] * X[n, s]) * scale
    
    # Initialize accumulator
    acc = tl.zeros((BLOCK_M, BLOCK_N), dtype=tl.float32)
    
    # Loop over samples dimension
    for s_start in range(0, N_samples, 32):
        s_offsets = s_start + tl.arange(0, 32)
        s_mask = s_offsets < N_samples
        
        # Load error[m, s]
        error_offsets = m_offsets[:, None] * stride_em + s_offsets[None, :] * stride_es
        m_valid = (m_offsets < M)[:, None]
        error_mask = m_valid & s_mask[None, :]
        error = tl.load(error_ptr + error_offsets, mask=error_mask, other=0.0)
        
        # Load X[n, s]
        x_offsets = n_offsets[:, None] * stride_xn + s_offsets[None, :] * stride_xs
        n_valid = (n_offsets < N_features)[:, None]
        x_mask = n_valid & s_mask[None, :]
        x = tl.load(X_ptr + x_offsets, mask=x_mask, other=0.0)
        
        # Accumulate: acc[m, n] += error[m, s] * X[n, s]
        # Need to reshape for proper broadcasting
        # error is (BLOCK_M, 32), x is (BLOCK_N, 32)
        # Want result (BLOCK_M, BLOCK_N)
        acc += tl.dot(error, tl.trans(x))
    ...
    valid = m_mask & n_mask
    grad_offsets = m_offsets[:, None] * stride_gm + n_offsets[None, :] * stride_gn
    tl.store(grad_ptr + grad_offsets, grad, mask=valid)


def sparse_gradient_computation(W, X, Y, mask, block_size=32):
    """
    Compute gradient sparsely: grad = 2 * (W @ X - Y) @ X.T / n_samples
    
    Only computes gradient blocks where mask is non-zero.
    This is the KEY to getting real speedup from sparse backward!
    
    Args:
        W: Weight matrix (M x N_features)
        X: Input matrix (N_features x N_samples)
        Y: Target matrix (M x N_samples)
        mask: Sparsity mask (M x N_features)
        block_size: Block size for Triton kernel
    
    Returns:
        Sparse gradient (M x N_features)
    """
    M, N_features = W.shape
    N_samples = X.shape[1]
    
    # Forward pass: compute prediction and error
    pred = W @ X  # (M x N_samples)
    error = pred - Y  # (M x N_samples)
    error = 2.0 * error  # Scale for MSE gradient
    
    # Allocate gradient
    grad = torch.empty_like(W)
    
    # Launch sparse gradient kernel
    grid = (triton.cdiv(M, block_size), triton.cdiv(N_features, block_size))
    
    sparse_grad_kernel[grid](
        grad,
        error, X, mask,
        M, N_features, N_samples,
...
def train_sparse_triton_fused(problem, n_steps=100, lr=0.01, block_size=32):
    """
    Train with sparse gradient + sparse Adam (fused means using sparse_adam_2d_kernel
    which expects pre-masked gradients).
    
    NOTE: "Fused" here means the gradient is already sparse when passed to Adam,
    so the Adam kernel doesn't need to mask again.
    """
    ...
    print("  (sparse gradient computation + sparse Adam optimizer)")
    start_total = time.time()
    ...
    for step in range(1, n_steps + 1):
        ...
        # Compute gradient SPARSELY (this is the key!)
        grad = sparse_gradient_computation(W, problem.X_train, problem.Y_train, 
                                          problem.mask, block_size=block_size)
        ...
        # Sparse Adam (gradients already sparse, so use the non-masking version)
...
def train_sparse_triton_separate(problem, n_steps=100, lr=0.01, block_size=32):
    """
    Train with DENSE PyTorch gradient + sparse Triton Adam optimizer.
    
    This isolates the optimizer speedup by using standard dense gradient computation.
    Useful for comparison to see the benefit of sparse gradient computation.
    """
    W = torch.randn_like(problem.W_true)
    ...
    print("  (DENSE PyTorch gradient + sparse Triton Adam optimizer)")
    start_total = time.time()
    ...
    for step in range(1, n_steps + 1):
        ...
        # Compute gradient DENSELY using PyTorch
...
def compare_results(problem, results_dense, results_fused, results_separate, results_pytorch):
    ...
    print(f"  Sparse Triton FUSED:       {fused_time*1000:6.3f}ms  ({baseline_time/fused_time:.2f}x) [sparse grad + sparse Adam]")
    print(f"  Sparse Triton SEPARATE:    {separate_time*1000:6.3f}ms  ({baseline_time/separate_time:.2f}x) [dense grad + sparse Adam]")
    print(f"  Sparse PyTorch:            {pytorch_time*1000:6.3f}ms  ({baseline_time/pytorch_time:.2f}x) [dense grad + dense Adam]")
    
    print(f"\nSpeedup Analysis:")
    print(f"  FUSED vs Dense:            {baseline_time/fused_time:.2f}x (full sparse pipeline)")
    print(f"  SEPARATE vs Dense:         {baseline_time/separate_time:.2f}x (optimizer only)")
    print(f"  PyTorch vs Dense:          {baseline_time/pytorch_time:.2f}x (gradient masking only)")
    print(f"  FUSED vs SEPARATE:         {separate_time/fused_time:.2f}x (gradient computation benefit)")
    print(f"  SEPARATE vs PyTorch:       {pytorch_time/separate_time:.2f}x (optimizer benefit)")
...
def main():
    ...
    # Test configurations
    configs = [
        {'size': 2560, 'sparsity': 0.8, 'block_size': 32, 'n_steps': 100},
        {'size': 2560, 'sparsity': 0.85, 'block_size': 32, 'n_steps': 100},
        {'size': 2560, 'sparsity': 0.9, 'block_size': 32, 'n_steps': 100},
        {'size': 2560, 'sparsity': 0.95, 'block_size': 32, 'n_steps': 100},
    ]
    ...
    print("KEY INSIGHTS")
    print("="*80)
    print("1. Sparse gradient computation is the KEY to real speedups")
    print("2. Fused kernel should be fastest (one launch vs two)")
    print("3. Both sparse methods skip computation for masked blocks")
    print("4. PyTorch baseline computes full dense gradients (slow)")
    print("5. Expected speedup ~ (1 - sparsity) for gradient + optimizer")
