#!/usr/bin/env python3
"""
Triton-Accelerated Sparse GRPO Training - OPTIMIZED VERSION
...
Based on sparse_DPO_v2.py but adapted for GRPO training with OpenR1 dataset.

Run: python src/magic/sparse_GRPO_v2.py \
  --checkpoint checkpoints_gemma3_grpo/checkpoint-100 \
...
def train(
    ...
    learning_rate=5e-5,
    subset_size=10,
    run_name="triton_sparse_grpo_optimized",
    mlp_only=True,
    block_size=BLOCK_SIZE,
    model=None,
    grpo_dataset=None,
    ...
    print(f"\n{'='*60}")
    print(f"TRITON-ACCELERATED SPARSE GRPO TRAINING (OPTIMIZED)")
    print(f"{'='*60}")
    ...
    # Set up GRPOTrainer - match baseline exactly
    print("\nInitializing GRPOTrainer with optimized SparseAdamW...")
    ...
    grpo_trainer.optimizer = sparse_optimizer
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Optimized Triton-accelerated sparse GRPO training",
    ...
    parser.add_argument("--run_name", type=str, default="triton_sparse_grpo_optimized", 
                       help="Run name for wandb (default: triton_sparse_grpo_optimized)")
