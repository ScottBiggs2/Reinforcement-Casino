...
```bash
python src/cold_start/mask_interpretation_suite.py --help
```
...
**Mask list file:** pass `MASK_SUITE_LIST_FILE` (one `.pt` path per line) plus `MASK_SUITE_OUT_DIR`, and optionally `MASK_SUITE_RUN_CKA=1` (CKA needs a GPU).

**Smoke / debug (two iid random masks):** `--smoke-debug --smoke-reference ANY_SAME_ARCH.pt` (seeds default `10001` / `20002`; sparsity default **97.5**). Forces extended Jaccard, heatmap, effective rank, `plot_layer_metrics` with MC band, and CKA unless `--smoke-no-cka`.
...
**GRPO Llama 3.1 8B (mag / oracle / random):** pass the three mask paths to the suite via `MASK_SUITE_LIST_FILE`.
...
`src/cold_start/mask_interpretation_suite.py` runs the full comparison as a single
