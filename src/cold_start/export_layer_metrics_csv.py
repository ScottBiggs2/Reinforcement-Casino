...
def cka_value_for_layer_row(
    ...
    """CKA scalar for one CSV row: hook/broadcast resolution, then direct lookup."""
...
def effective_rank(mask_tensor: torch.Tensor, eps: float = 1e-12) -> Tuple[Optional[float], Optional[float]]:
    ...
    # Full `svdvals` on the 2D mask slice (plots + behavior validated against the reference).
